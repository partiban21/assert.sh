...
test_assert_file_exist() {
  ...
  assert_file_exist "$tmp_non_existing_dir"
  if [ "$?" == 1 ]; then
    log_success "assert_file_exist returns 1 if param is a directory"
  else
    log_failure "assert_file_exist does not work"
  fi
...
test_assert_file_not_exist() {
  ...
  assert_file_not_exist "$tmp_non_existing_file"
  if [ "$?" == 0 ]; then
  ...
  assert_file_not_exist "$tmp_existent_dir"
  if [ "$?" == 0 ]; then
    log_success "assert_file_not_exist returns 1 if param is a directory"
  else
    log_failure "assert_file_not_exist does not work"
  fi
